...
def submit_live_attendance(stu_id, student_class,date):
    attendance_class={
        ...
        "xiii": "INSERT INTO xiii(student_id,attendance_date) VALUES(?, ?);",
        "iv": "INSERT INTO iv(student_id,attendance_date) VALUES(?, ?);",
    }
...
def fetchAttendance(attendanceClass, attendanceDate):
    student_class={
        ...
        'xiii': "SELECT DISTINCT(d.student_id),d.fullname,d.class,ac.attendance_date FROM xiii ac,student_data d WHERE ac.student_id=d.student_id AND attendance_date = ?;",
        'iv': "SELECT DISTINCT(d.student_id),d.fullname,d.class,ac.attendance_date FROM iv ac,student_data d WHERE ac.student_id=d.student_id AND attendance_date = ?;",
    }
    conn = create_connection()
    cursor = conn.cursor()
    val = [attendanceDate]
...
def fetch_graph_data(graphClass):
    student_class = {
        ...
        'xiii': "SELECT DISTINCT(attendance_date) FROM xiii ORDER BY attendance_date ASC LIMIT 06 ",
        'iv': "SELECT DISTINCT(attendance_date) FROM iv ORDER BY attendance_date ASC LIMIT 06 ",
    }
    attendance_class = {
        ...
        'xiii': "SELECT COUNT(DISTINCT(student_id)) FROM xiii WHERE attendance_date = ? ;",
        'iv': "SELECT COUNT(DISTINCT(student_id)) FROM iv WHERE attendance_date = ? ;",
    }
